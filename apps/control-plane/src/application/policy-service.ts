...
import { updatePolicy } from "../infrastructure/policy-repository";
...
export async function updateManagedPolicy(input: CreatePolicyInput) {
  const updated = await updatePolicy(input);

  if (!updated) {
    throw new Error(`Policy "${input.id}" was not found.`);
  }
